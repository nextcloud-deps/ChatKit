...
public class MessagesListAdapter<MESSAGE extends IMessage>
   ...
   * will be used.
   */
  public String copySelectedMessagesText(Context context, Formatter<MESSAGE> formatter,
      boolean reverse) {
    String copiedText = getSelectedText(formatter, reverse);
   ...
   * @param onMessageViewClickListener click listener.
   */
  public void registerViewClickListener(int viewId,
      OnMessageViewClickListener<MESSAGE> onMessageViewClickListener) {
  ...
  public int getSameAuthorLastMessagesCount(String id) {
    ...
    for (int i = 0; i < items.size(); i++) {
      ...
      if ((item.getUser().getId().contentEquals(id) && TextUtils.isEmpty(
          item.getSystemMessage()))) {
        count++;
      } else {
        return count;
      }
